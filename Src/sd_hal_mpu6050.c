...
//setar em 1 para ativar o DLPF, em 0 para desativar
#define DLPF 1
/* Default I2C address */
...
SD_MPU6050_Result SD_MPU6050_Init(I2C_HandleTypeDef* I2Cx, SD_MPU6050* DataStruct, SD_MPU6050_Device DeviceNumber, SD_MPU6050_Accelerometer AccelerometerSensitivity, SD_MPU6050_Gyroscope GyroscopeSensitivity)
{
	...
	//------------------
		/* Send address */
		if(HAL_I2C_Master_Transmit(Handle, address, &WHO_AM_I, 1, 10) != HAL_OK)
		{
			return SD_MPU6050_Result_Error;
		}

		/* Receive multiple byte */
		if(HAL_I2C_Master_Receive(Handle, address, &temp, 1, 10) != HAL_OK)
		{
			return SD_MPU6050_Result_Error;
		}
	...
	//------------------
		...
		if(HAL_I2C_Master_Transmit(Handle,(uint16_t)address , (uint8_t *)d, 2, 10) != HAL_OK)
		{
					return SD_MPU6050_Result_Error;
		}
	//------------------
	...
	if(DLPF){
		/* Set DLPF */
		SD_MPU6050_SetDLPF(I2Cx,DataStruct,5);
		/* Set sample rate to 1kHz */
		SD_MPU6050_SetDataRate(I2Cx,DataStruct, (SD_MPU6050_DataRate_1KHz+1)/8);
	}
	else{
	...
	//buffer[0] = 6;
	//while (HAL_I2C_Mem_Write(Handle,(uint16_t)0x68<<1, (uint16_t)0x1A, 1, 6, 1, 100) != HAL_OK);//Configura o low pass filter interno do MPU como 6Hz
...
SD_MPU6050_Result SD_MPU6050_SetDLPF(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, uint8_t bandwidth)
{
	uint8_t d[2];
	I2C_HandleTypeDef* Handle = I2Cx;
	uint8_t address = DataStruct->Address;
	...
	while(HAL_I2C_Master_Transmit(Handle,(uint16_t)address,(uint8_t *)d, 2, 100)!=HAL_OK);
	/*{
				return SD_MPU6050_Result_Error;
	}*/
...
SD_MPU6050_Result SD_MPU6050_SetDataRate(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, uint8_t rate)
{
	...
	/* Set data sample rate */
	while(HAL_I2C_Master_Transmit(Handle,(uint16_t)address,(uint8_t *)d, 2, 10)!=HAL_OK);
	/*{
				return SD_MPU6050_Result_Error;
	}*/
...
SD_MPU6050_Result SD_MPU6050_SetAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Accelerometer AccelerometerSensitivity)
{
	...
	/* Config accelerometer */
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address,&regAdd, 1, 10) != HAL_OK);
	/*{
				return SD_MPU6050_Result_Error;
	}*/
	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, &temp, 1, 10) != HAL_OK);
	/*{
				return SD_MPU6050_Result_Error;
	}*/
	temp = (temp & 0xE7) | (uint8_t)AccelerometerSensitivity << 3;
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address,&temp, 1, 10) != HAL_OK);
	/*{
				return SD_MPU6050_Result_Error;
	}*/
...
SD_MPU6050_Result SD_MPU6050_ReadAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address, &reg, 1, 10) != HAL_OK);

	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 6, 10) != HAL_OK);
...
SD_MPU6050_Result SD_MPU6050_ReadTemperature(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 2, 10) != HAL_OK);
...
SD_MPU6050_Result SD_MPU6050_ReadAll(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 14, 10) != HAL_OK);
...
SD_MPU6050_Result SD_MPU6050_EnableInterrupts(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address, reg, 2, 10) != HAL_OK);

	uint8_t mpu_reg= MPU6050_INT_PIN_CFG;
	/* Clear IRQ flag on any read operation */
	while(HAL_I2C_Master_Transmit(Handle, (uint16_t)address, &mpu_reg, 1, 10) != HAL_OK);

	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, &temp, 14, 10) != HAL_OK);
	temp |= 0x10;
...
SD_MPU6050_Result SD_MPU6050_ReadInterrupts(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Interrupt* InterruptsStruct)
{
	...
	while(HAL_I2C_Master_Receive(Handle, (uint16_t)address, &read, 14, 10) != HAL_OK);
