...
void canMessageReceived(uint16_t id, uint8_t* data)
{
	...
	switch(id)
	{
		...
		case 104:
			...
			break;
		case 105:
			Speed_RR = 		data_word[0];
			Speed_LR = 		data_word[1];
			break;
		case 106:
...
void writeSD(void)
{
	...
	int len = sprintf(buffer_log, "\r\n%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%d\t%d\t%u\t%u\t%d\t%d\t%u\t%u\t%u\t%u\t%u\t%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%u\t%u\t%u\t%d\t%d\t%d\t%d",ECU_Mode, Intensidade_Frenagem, _ecu_flag ,timer,Hodometro_P, Hodometro_T, Speed_LR, Speed_RR,V_motor_D, V_motor_E, Torque_LM, Torque_RM, Torque_ref_R, Torque_ref_L, Current_LM, Current_RM, Pedal, Volante, tempInv_D1, tempInv_D2, tempInv_E1, tempInv_E2, accelX, accelY, accelZ, gyroX, gyroY, gyroZ, temp, sensorpressao1, leitura_PotInt, tempInt, ext1, ext2, ext13, ext23, ext22,leitura_PotInt2, PotTD, sensorpressao2, leitura_PotInt2, tempInt2, current_sensor1_baixa, current_sensor1_alta, current_sensor2, current_sensor3, tensaototal, tempmediabb, tempmaxbb, temp_pack0_1, temp_pack0_2, temp_pack1_1, temp_pack1_2, temp_pack2_1, temp_pack2_2, temp_pack3_1, temp_pack3_2, temp_pack4_1, temp_pack4_2, temp_pack5_1, temp_pack5_2,Current_BAT, Volt_BAT, Tensao_GLV, IRCan[0],IRCan[1],IRCan[2],IRCan[3]);
